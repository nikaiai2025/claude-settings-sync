...
### 1. テンプレートからリポジトリを作成
1. このリポジトリ上部の **"Use this template"** → **"Create a new repository"** をクリック
2. **Private** を選択（個人設定が含まれるため）
3. リポジトリ名を任意に設定（例: `my-claude-settings`）
4. **"Create repository"** をクリック

### 2. ローカルにクローン
```bash
git clone https://github.com/<you>/my-claude-settings.git
cd my-claude-settings
```

### 3. 初回同期
```bash
python claude_settings_sync.py status        # 現在の状態を確認
python claude_settings_sync.py local_to_git  # ローカル設定を Git に取り込み
```
...
git clone https://github.com/<you>/my-claude-settings.git
cd my-claude-settings
python claude_settings_sync.py git_to_local  # Git の設定をローカルに反映
```
...
スクリプトの新しいバージョンがリリースされた場合:
```bash
curl -o claude_settings_sync.py https://raw.githubusercontent.com/nikaiai2025/claude-settings-sync/main/claude_settings_sync.py
```
